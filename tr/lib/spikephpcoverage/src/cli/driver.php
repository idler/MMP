...
<?php
    ...
    */
    ...
    function usage() {
        ...
        echo "     report options \n";
        echo "       --cov-data-file <path>                    Coverage data file path [use this instead of --cov-url for a local file path]\n";
    ...
    for ($i = 1; $i < $_SERVER["argc"]; $i++) {
        ...
        case "--cov-file-name":
            ...
            break;
        case "--cov-data-file":
            $OPTION['cov-data-file'] = $_SERVER['argv'][++$i];
            break;
        case "--report-name":
    ...
    if (!$OPTION['cov-url']){
        if(!$OPTION['report'] && !$OPTION['cov-data-file']) {
    ...
    if ($OPTION['init']){
        ...
        if($OPTION['cov-data-file']) {
            if(!is_readable($OPTION['cov-data-file'])) {
                echo "Error: Cannot read cov-data-file: " . $OPTION['cov-data-file'] . "\n";
                exit(1);
            }
            $xmlUrl = $OPTION['cov-data-file'];
        }
        else {
